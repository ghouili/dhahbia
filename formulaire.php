...
$ensignant =  $matiere = $niveau = $filiere = $auditoire = $nb_etudient = $nb_page = $date = $type_imp = "";
$ensignant_err = $matiere_err = $niveau_err = $filiere_err = $auditoire_err = $nb_etudient_err = $nb_page_err = $date_err = $type_imp_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($ensignant_err) && empty($matiere_err) && empty($niveau_err) && empty($filiere_err) && empty($auditoire_err) && empty($nb_etudient_err) && empty($nb_page_err) && empty($date_err) && empty($type_imp_err)) {
        ...
        $sql = "INSERT INTO impression (ensignant, matiere, niveau,filiere,auditoire,nb_etudient,nb_page, date ,type_imp, doc) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        if ($stmt = mysqli_prepare($link, $sql)) {

            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssisssssss", $ensignant, $matiere, $niveau, $filiere, $auditoire, $nb_etudient, $nb_page, $date, $type_imp, $doc);
